---
title: AI Translation Workflow Documentation
---
...
* `.po` files:
  `my-plugin-textdomain-fr_FR-*.po`, `my-plugin-textdomain-es_ES-*.po`, etc.
...
* Always extract strings using WordPress i18n functions: `__()`, `_e()`, `_x()`, etc.
* Use context where possible (`_x()`), especially for ambiguous terms.
* Organize translation files per locale and per text domain.
* Avoid duplicating strings with different whitespace or punctuation — they’ll be treated separately.
